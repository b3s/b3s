...
RSpec.describe "ExchangePosts" do
  subject { response }
  ...
  describe "GET /discussions/:discussion_id/posts/since" do
    context "with posts" do
      ...
      before { get since_discussion_posts_path(exchange, index: 2) }
    ...
    context "when discussion does not exist" do
      before { get since_discussion_posts_path(999_999, index: 0) }

      it { is_expected.to have_http_status(:not_found) }
    end
  end

  describe "GET /conversations/:conversation_id/posts/since" do
    context "when conversation is not viewable" do
      let(:exchange) { create(:conversation) }

      before { get since_conversation_posts_path(exchange, index: 0) }
  ...
  describe "GET /discussions/:id/posts/search" do
    let(:query) { nil }
    let!(:matching_post) do
      create(:post, exchange:, body: "findme unique content")
    end

    before { get search_discussion_posts_path(exchange, q: query) }
    ...
    context "when discussion does not exist" do
      before { get search_discussion_posts_path(999_999, q: "test") }

      it { is_expected.to have_http_status(:not_found) }
    end
  end

  describe "GET /conversations/:id/posts/search" do
    let(:exchange) { create(:conversation) }
    let(:participant) { exchange.poster }
    let(:user) { participant }
    let(:query) { nil }
    let!(:matching_post) do
      create(:post, exchange:, user: participant,
                    body: "findme unique content")
    end

    before { get search_conversation_posts_path(exchange, q: query) }
    ...
    context "when conversation does not exist" do
      before { get search_conversation_posts_path(999_999, q: "test") }
